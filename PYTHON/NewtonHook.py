...
def NewtonHook(f, df, dfp, sub, dp, m, n, gtol,tol, delta, mndl, mxdl, nits, info):
   new_x	= settings.new_x	# current best guess, x
   new_fx = settings.new_fx	#  f(x)
   new_tol = settings.new_tol	# |f(x)|
   new_del = settings.new_del# current size of trust region
   new_nits = settings.new_nits	# number of newton iterations taken
   new_gits	= settings.new_gits # number of GMRES iterations taken
   ...
   ginfo    = info 
   new_fx   = f(n,new_x) 
   new_tol  = np.sqrt(dp(n,new_fx,new_fx)) 
   if delta<0E0:  
      new_del = new_tol / 10E0 
      mxdl_   = 1E99
   
   if info==1: 
      print('newton: nits={}  res={}\n'.format( new_nits, new_tol))
   ...
   while 1:
      ...
      if del==0E0:
         new_gits = 9999 
      [s,gres,gdel,new_gits,ginfo] = GMRESm(m,n,s,fx_,df,dfp,dp,gres,gdel,new_gits,ginfo) 
      ginfo = info 
      new_x = x_ - s 
